...
function metricsHandler(_req, res) {
  const lines = [
    ...
    `detour_process_uptime_seconds{service="auth-service"} ${Number(process.hrtime.bigint() - startedAt) / 1e9}`,
  ...
  requests.forEach((entry, key) => {
    ...
    const labels = `service="auth-service",method="${escapeLabel(method)}",route="${escapeLabel(route)}",status="${escapeLabel(status)}"`;
