...
function metricsHandler(_req, res) {
  const lines = [
    ...
    `detour_process_uptime_seconds{service="routing-service"} ${Number(process.hrtime.bigint() - startedAt) / 1e9}`,
  ...
  requests.forEach((entry, key) => {
    ...
    const labels = `service="routing-service",method="${escapeLabel(method)}",route="${escapeLabel(route)}",status="${escapeLabel(status)}"`;
